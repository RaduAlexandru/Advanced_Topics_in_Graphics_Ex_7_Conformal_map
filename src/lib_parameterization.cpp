...
void detect_boundary_vertices(const Eigen::MatrixXd &points, const Eigen::MatrixXi &faces, Eigen::VectorXi &boundary_indices, Eigen::VectorXi &interior_indices, Eigen::MatrixXi &boundary_edges)
{
  ...
  igl::boundary_loop(faces,boundary);
  //get the boundry as an eigen vector
  boundary_indices=Eigen::Map<Eigen::VectorXi>(boundary.data(), boundary.size());


  //interior vertices
  Eigen::VectorXi indices_all;
  igl::colon(0,faces.maxCoeff(), indices_all);
  Eigen::VectorXi other;
  igl::setdiff(indices_all,boundary_indices,interior_indices,other);


  std::cout << "boundry_indices is " << boundary_indices << '\n';



  //edges the way they want it
  boundary_edges.resize(boundary_indices.rows(),2);
  for (size_t i = 0; i < boundary_indices.rows()-1; i++) {
    std::cout << "i" << i << '\n';
    boundary_edges(i,0)=boundary_indices(i);
    boundary_edges(i,1)=boundary_indices(i+1);
  }
  boundary_edges(boundary_indices.rows()-1,0)=boundary_indices(boundary_indices.rows()-1);
  boundary_edges(boundary_indices.rows()-1,1)=boundary_indices(0);
  // std::cout << "boundary edges is" << "\n" << boundary_edges << '\n';



  // //edges
  // boundary_edges.resize(boundary_indices.size(),3);
  // for (size_t i = 0; i < boundary_indices.size(); i++) {
  //   boundary_edges.row(i) = points.row(boundary[i]);
  // }
...
Eigen::VectorXd path_length(const Eigen::MatrixXi &path, const Eigen::MatrixXd &points)
{
    ...
    for (size_t i = 0; i < path.rows(); i++) {
      Eigen::VectorXd p1 = points.row(path(i,0));
      Eigen::VectorXd p2 = points.row(path(i,1));
      Eigen::VectorXd edge= p1-p2;
      edge_lengths(i)=edge.norm();
    }
...
Eigen::MatrixXi detect_boundary_path(const Eigen::MatrixXi &boundary_edges)
{
    ...
    return boundary_edges;  //TODO we're returning the input because it should be already sorted
}
...
Eigen::MatrixXd map_boundary_edges_to_circle(const Eigen::VectorXd &edge_lengths)
{
    ...
    float sum_edge_lengths=0;
    sum_edge_lengths=edge_lengths.sum();
    std::cout << "sum_edge_lengths " << sum_edge_lengths << '\n';
